...
The installation instrucutions are detailed here. For convience, installation scripts have been written to install the software for choice operating systems. Currently, the supported operating systems are, in order, Rocky Linux, Windows and Ubuntu. Future compatability with other operating systems are unlikely to be supported in the future.

Automated Scripts
=================

As there are many parts to the OpihiExarata software, for convience, an installation script has been written for convience and to help facilitate installation. Please note that the automated script installs everything, if you want to customize your install, please see :ref:`technical-installation-manual-installation`.

The installation script is written in Powershell Core and thus require the installation of Powershell Core. Instrucutions for the installation and usage of `Powershell Core <https://docs.microsoft.com/en-us/powershell/>`_ can be found on its website. The installation of the OpihiExarata software does not depend on Powershell Core after installation, but the convience scripts would be unavaliable. If this is the case, please follow the other installation instrucutions as usual.

In addition to installation script, an auxillary script has been written in Powershell Core. The purpose of the auxillary script is to do tasks not strictly related to installation. These tasks include code formatting, testing, and documentation building. You may use this as well but it is not nessiasry for installation.
...
- Like the install script, you need to specify the version-specific Python command that Powershell Core uses. Typically it is typically something like ``python3.9`` for Python 3.9. It is adivsed that you use the same entry as in the installation script::
...
2.  :ref:`technical-installation-python-part`: You will download a supported Python version and build then install the Python part of OpihiExarata and its dependcies. This part being the primary part.
