...
class PropagativeSolution(library.engine.ExarataSolution):
    ...
    def __init__(
        self: PropagativeSolution,
        ra: hint.array,
        dec: hint.array,
        obs_time: list,
        solver_engine: hint.PropagationEngine,
        vehicle_args: dict = None,
    ) -> None:
        """Instantiate the propagation solution.
        ...
        vehicle_args : dictionary, default = None
            ...
            they are given here. By default, no arguments.
        ...
        if isinstance(solver_engine, library.engine.PropagationEngine):
            raise error.EngineError(
                "The propagation solver engine provided should be the engine "
            ...
            raise error.EngineError(
                "The provided propagation engine is not a valid engine which "
                "can be used for propagation solutions.",
            )
        ...
        try:
            ...
            self._propagation_function = raw_propagate_results[
                "propagation_function"
            ]
        except KeyError:
            raise error.EngineError(
                "The engine results provided are insufficient for this "
                "propagation solver. Either the engine cannot be used because "
    ...
    def __init_compute_raw_motion(
        ...
        # observations.
        accel_min_obs = 3
        if unix_obs_time_array.size >= accel_min_obs:
            ...
            delta2_time = (
                unix_obs_time_array[2:] - unix_obs_time_array[:-2]
            ) / 2
    ...
    def __init_compute_propagation_motion(
        ...
        propagate_ra_velocity : float
            The propagative right ascension angular velocity of the target,
            in degrees per second.
        propagate_dec_velocity : float
            The propagative declination angular velocity of the target,
            in degrees per second.
        propagate_ra_acceleration : float
            The propagative right ascension angular acceleration of the target,
            in degrees per second squared.
        propagate_dec_acceleration : float
            The propagative declination angular acceleration of the target,
            in degrees per second squared.
    ...
    def forward_propagate(
        ...
        """Find the new coordinates via propagation.
...
def _vehicle_linear_propagation(
    ...
    polyprop = propagate.LinearPropagationEngine(
        ra=ra_array,
        dec=dec_array,
        obs_time=obs_time_array,
    )
    # Check that the system uses quadratics, otherwise this section needs to
    # be double checked.
    if polyprop.ra_poly_param.shape != (
        2,
    ) or polyprop.dec_poly_param.shape != (2,):
        raise error.DevelopmentError(
            "The polynomial engine does not use quadratics. This engine "
            "function needs to be revised to ensure that it is returning "
            "the correct values.",
        )

    # The propagation function.
    def propagation_function(t: hint.ndarray) -> hint.ndarray:
        """Evaluate the forward propagation."""
        return polyprop.forward_propagate(future_time=t)
...
def _vehicle_quadratic_propagation(
    ...
    if polyprop.ra_poly_param.shape != (
        3,
    ) or polyprop.dec_poly_param.shape != (3,):
        raise error.DevelopmentError(
            "The quadratic engine does not use quadratics. This engine "
            "function needs to be revised to ensure that it is returning "
            "the correct values.",
        )
