...
def get_most_recent_filename_in_directory(directory:str, extension:hint.Union[str,list]=None) -> str:
    ...
    if not os.path.isdir(directory):
        raise error.InputError("The directory provided `{d}` does not exist. A most recent file cannot be obtained.".format(d=str(directory)))
    ...
    for extensiondex in extension_list:
        ...
        pathname_glob_filter = merge_pathname(directory=directory, filename="*", extension=extensiondex)
    ...
    if not os.path.isfile(recent_filename):
        raise error.DevelopmentError("For some reason, the detected most recent file is not actually a file. Something is wrong.")
